...
struct EventToast {
  ...
  // When true, this toast is generated by a UI helper (e.g., Watchboard alerts)
  // rather than a SimEvent.
  bool custom{false};

  // Optional: Watchboard alert context (for jump/copy actions).
  std::uint64_t watch_id{0};
  std::string watch_label;
  std::string watch_path;
  // For query-style pins, a representative concrete pointer (usually the first match).
...
struct HUDState {
  ...
  std::uint64_t next_custom_toast_seq{1};
